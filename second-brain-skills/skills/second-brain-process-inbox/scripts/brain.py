...
def config(path):
    ...
    if c['version'] not in ('2','3') or c['timezone'] != 'Europe/Warsaw':
        fail('Expected version 2 or 3 and timezone Europe/Warsaw')
    ...
    for r in c['repositories']:
        ...
        if set(r) - {'branch'} != {'id', 'path', 'gitlab_host', 'gitlab_project', 'remote'}:
        ...
        ids.add(r['id'])
        if c['version']=='3' and not r.get('branch'): fail('Each repository requires branch in version 3')
        if r.get('branch') and (r['branch'].startswith('-') or any(x.isspace() for x in r['branch'])): fail('Invalid branch')
...
def active(root):
    ...
    for directory, dirs, files in os.walk(root, followlinks=False):
        dirs[:] = [d for d in dirs if d not in ('archive', '.state', '.git', 'thoughts')
...
def publish(c, plan):
    ...
    if (root/'.state/actualize-publishing.json').exists(): fail('Run actualize recover first')
    if stage not in ('daily','knowledge'): fail('Invalid stage')
...
def main():
    ...
    if (root/'.state/bootstrap-publishing.json').exists() and args.command not in ('read','check'):
        fail('Interrupted bootstrap publication; run bootstrap recover first')
    if (root/'.state/actualize-publishing.json').exists() and args.command not in ('read','check'): fail('Run actualize recover first')
    if args.command == 'check': result = {'valid': True, 'repositories': len(c['repositories'])}
        ...
        if any(p.is_relative_to(root/d) for d in ('archive','thoughts')): fail('Archive and thoughts are outside this workflow')
        raw = p.read_bytes(); result = {'path': args.path, 'expected_sha256': hashlib.sha256(raw).hexdigest(), 'content': raw.decode()}
    else:
        with lock(root):
            if args.command == 'init':
                for d in ('inbox','personal','archive','knowledge/domain','knowledge/technical/shared','knowledge/technical/unknown','conflicts','architecture/cross-repository','architecture/unknown'):
