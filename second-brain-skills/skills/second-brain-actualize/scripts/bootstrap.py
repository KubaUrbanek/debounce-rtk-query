...
class Bootstrap:
    ...
    def editable(self, state):
        ...
        if state['status'] == 'completed': brain.fail('Run prepare to start the next synchronization')
    ...
    def publish(self):
        ...
        targets=set(brain.active(self.root))|{self.root/'index.md'}|{brain.safe(self.root,d['path']) for d in s['draft']['documents']}|{brain.safe(self.root,d['path']) for d in s['draft'].get('delete_documents',[])}
        ...
        try:
            removed={brain.safe(self.root,d['path']) for d in s['draft'].get('delete_documents',[])}
            for d in s['draft']['documents']: tracked_write(brain.safe(self.root,d['path']),d['content'])
            for path in removed:
                name=str(path.relative_to(self.root))
                journal['allowed_hashes'].setdefault(name,[]).append(None)
                write_json(self.journal,journal)
                path.unlink()
            if removed:
                for path in brain.active(self.root):
                    original=path.read_text()
                    def repair(match):
                        target=brain.targets(path,match.group(0))
                        return match.group(1) if any(t in removed for t in target) else match.group(0)
                    updated=brain.LINK.sub(repair,original)
                    if updated!=original: tracked_write(path,updated)
