...
```bash
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID organize --plan AREAS_JSON
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID findings
```
...
```bash
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID tasks
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID claim --worker FRESH_AGENT_ID --role explorer
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID task-start --task TASK_ID --worker FRESH_AGENT_ID
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID task-finish --task TASK_ID --worker FRESH_AGENT_ID --result RESULT_JSON
```
...
`prepare` reads `branch` from YAML. It imports the commit in a completed old
`.state/bootstrap/REPO_ID/state.json` when no actualization state exists; that commit is
already documented. Old state remains unchanged. A malformed/missing legacy commit or an
invalid ancestor requests an explicit user baseline. An incomplete legacy bootstrap blocks
with an explanation rather than pretending that it has published complete knowledge.

Without either state, prepare starts a full current-snapshot analysis. Subsequent runs record
`baseline_commit`, pinned `commit`, cumulative `changes` and `commits` after the baseline.
Completed per-run evidence is retained internally in `runs/`; it is not the user's archive.
No-change results have `no_changes: true`. Preparing an interrupted run keeps its original
snapshot/results; resolve blocks and resume it before selecting a different branch.

```bash
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID baseline --commit FULL_SHA
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID inspect --path src/OrderService.java --revision target --start 0 --size 12000
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID inspect --path src/OrderService.java --revision baseline --start 0 --size 12000
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID diff --path src/OrderService.java --start 0 --size 12000
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID impact --plan IMPACT_JSON
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID expand --plan EXPANSION_JSON
```

`baseline` is only for answering an `awaiting_baseline` question, with a full valid ancestor SHA.
That SHA is excluded from the new range. Do not invent it. `inspect` and `diff` return bounded
text plus total length; read all ranges needed rather than silently truncating evidence.
They are for reconnaissance and before/after comparison, not substitutes for worker receipts.

Record impact before organize or submissions. Initial analysis requires `mode: full`:

```json
{"mode":"full","reason":"Initial scan or uncertain cross-cutting impact requires full coverage.","deletions":[]}
```

For incremental affected scope:

```json
{
  "mode":"affected",
  "reason":"Explain actual change impact, not just matching filenames.",
  "files":["src/OrderService.java","src/OrderEndpoint.java"],
  "dependency_analysis":"Explain reverse callers, registration, shared configuration and persistence reach.",
  "unaffected":[{"path":"src/Unrelated.java","reason":"Evidence-backed absence of affected dependency."}],
  "deletions":[{"path":"src/Removed.java","assessment":"Explain removed behavior and affected documentation/callers."}]
}
```

Use actual manifest paths. Include every changed non-excluded target path, all affected callers
and complete flow dependencies. Account for every omitted non-excluded target file in unaffected,
and every deleted source path in deletions, even if the deleted file was excluded by policy.
Do not omit a blocked document merely to pass coverage; resolve extraction or block for the user.
Expand with `{"files":["src/NewlyDiscoveredDependency.java"]}`. Expansion invalidates draft/review
and requeues completed tasks conservatively; revise organize to cover the expanded scope.

All publication plans also require `change_assessment` and `change_dispositions`:

```json
{
  "change_assessment":"Cumulative impact of the baseline-to-target change, including unchanged callers.",
  "change_dispositions":[{"path":"src/Removed.java","assessment":"Removed behavior; obsolete topic deleted and callers checked."}],
  "delete_documents":[{
    "path":"knowledge/domain/obsolete-topic.md",
    "expected_sha256":"HASH_FROM_READ",
    "reason":"No remaining current behavior described by this topic.",
    "other_repository_check":"Explain why deletion loses no other project variants."
  }]
}
```

Every changed path must have a disposition, including generated/test changes and deletions.
Deletes are limited to knowledge topics; essence and ADRs cannot be deleted. Existing incoming
links are converted to their label text, then reciprocal navigation is rebuilt. Update meaningful
surrounding prose in staged documents as well. Do not leave descriptions implying removed behavior.
The publisher journals deletions, writes and link repairs together. It never archives removed topics.

If no target source needs analysis (for example only excluded tests changed), an empty scope
is allowed after explicit impact reasoning. Stage `documents: []` if existing knowledge is
unchanged, plus `empty_scope_assessment`, `change_assessment`, and complete `change_dispositions`.
If all source was deleted, stage appropriate remaining-note edits and guarded deletions. Do not
fabricate chunks/findings. A fresh registered reviewer still checks actual before/after evidence,
impact, deletions, cross-repository preservation and every proposed output, with exact draft hash.
Only publication advances the checkpoint, even with zero document changes.

Every approved review additionally needs nonempty `impact_check`, `deletion_check` and
`cross_repository_check`. Describe concrete checks. For empty scope the normal source chunk
checks do not apply, but review of the impact argument and all output documents does.
...
```bash
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID prepare
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID status
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID split --chunk CHUNK_ID --size 4000
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID extract --path SOURCE_PATH --text-file UTF8_FILE --method "Static extraction method"
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID claim --worker AGENT_ID --role explorer
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID read --worker AGENT_ID --chunk CHUNK_ID
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID submit --worker AGENT_ID --result RESULT_JSON
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID release --worker AGENT_ID
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID stage --plan PLAN_JSON
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID claim --worker REVIEWER_ID --role reviewer
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID review --worker REVIEWER_ID --result REVIEW_JSON
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID publish
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID block --reason "Explanation"
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID unblock
bash SKILL_DIR/scripts/actualize.sh --config CONFIG --repo REPO_ID recover
```

State lives under `BRAIN_DIR/.state/actualize/REPO_ID/`. Snapshot storage is bare Git: no
...
empty, supply a specific top-level `no_flows_reason`. When no target chunks remain in scope, use the explicit empty-scope protocol above.

Shared notes may be updated with observed caller-side facts and reciprocal links; do not edit another repository’s technical notes.
...
approved independent review must pass first. The completion marker and checkpoint are written together only on success.
