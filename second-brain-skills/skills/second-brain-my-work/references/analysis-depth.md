...
Independent review is required only for actualization; other skills perform this review themselves
and retain their one-agent execution contract.

## Workflow-specific application

- **Actualization:** discover actual entry points first (endpoints, message/Kafka consumers, cron/
  ...
  across unrelated jobs. These delegation instructions apply to actualization only.
...
Every writer also reads relevant existing notes and adds justified reciprocal relationships.
Do not introduce technical language into domain-side link explanations.
